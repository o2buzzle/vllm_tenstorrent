...
class TTModelRunner(ModelRunnerBase[TTModelInput]):
    ...
    def _execute_model_single_step(self,
        ...
        # hack to make sure that the model input is ttnn
        device_kv_caches = None
        if kv_caches is not None:
            device_kv_caches = []
            for kv_cache in kv_caches:
                if isinstance(kv_cache, torch.Tensor):
                    # Convert to ttnn tensor if needed
                    kv_cache = ttnn.to_ttnn(kv_cache)
                    device_kv_caches.append(kv_cache)
                else:
                    # Assume kv_cache is already a ttnn tensor
                    device_kv_caches.append(kv_cache)

        execute_model_kwargs = {
            "tokens": model_input.input_tokens,
            "page_table": model_input.block_tables,
            "kv_cache": device_kv_caches,
