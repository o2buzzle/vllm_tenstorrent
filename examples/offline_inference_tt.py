...
from register_models_tt import register_models_tt, check_tt_model_supported, generate_env

register_models_tt()  # Import and register models from tt-metal
...
def run_inference(
    ...
    if not measure_perf:
        if not multi_modal:
            ...
            assert isinstance(
                prompts, list), "Prompts must be a list of strings"
        else:
    ...
    if prompt_token_ids is not None:
        ...
        for single_prompt_token_ids in prompt_token_ids:
            prompts.append(TokensPrompt(
                prompt_token_ids=single_prompt_token_ids))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_tokens", type=int,
                        default=128, help="Length of outputs")
    parser.add_argument(
        "--greedy_sampling",
        action="store_true",
    ...
    parser.add_argument("--async_engine", action="store_true",
    ...
    parser.add_argument("--max_model_len", type=int,
                        default=None, help="Max model len")
    parser.add_argument(
